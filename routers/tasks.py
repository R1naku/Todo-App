...
async def list_tasks(current_user: TelegramUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def update_task(task_id: int, update_data: TaskUpdate, current_user: TelegramUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    update_dict = update_data.dict(exclude_unset=True)  # Только изменённые поля
